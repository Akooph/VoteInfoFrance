...
export default function MapLanding() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .or(`geo_code.eq.${selected.code},geo_level.eq.national,geo_level.eq.europeen`)
      ...
      {selected && (
        <div style={{
          ...
          {!loading && propositions.map((p, i) => (
            <a key={p.id} href={`/proposition/${p.id}`} style={{ display: 'block', textDecoration: 'none',
              ...
              <p style={{ margin: 0, fontSize: 11, color: '#9ca3af' }}>{p.institution.replace(/_/g, ' ')} · {p.status}</p>
            </a>
          ))}

          <a href="/dashboard" style={{ display: 'block', marginTop: 12, fontSize: 13, color: '#1d4ed8', textDecoration: 'none', fontWeight: 600 }}>
